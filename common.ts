...
import { readableStreamToString, DotsError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new DotsError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new DotsError(e, e.response?.data)
                }
                if (innerError instanceof DotsError) {
                    // Got "DotsError" from the above try block
                    throw innerError;
                }
